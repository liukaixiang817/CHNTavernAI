...
$("#rm_button_create").on("click", function () {                 //when "+New Character" is clicked
    ...
    $("#result_info").html('去聊天以开始Token统计');
});
...
export function RA_CountCharTokens() {
    ...
    if (count_tokens < 1024 && perm_tokens < 1024) {
        $("#result_info").html(count_tokens + " Tokens (" + perm_tokens + " 固定Tokens)");      //display normal if both counts are under 1024
    } else {
        $("#result_info").html(`
        <span class="neutral_warning">${count_tokens}</span>&nbsp; Tokens (<span class="neutral_warning">${perm_tokens}</span><span>&nbsp;固定Tokens)
        <br>
...
function RA_checkOnlineStatus() {
    if (online_status == "no_connection") {
        $("#send_textarea").attr("placeholder", "未连接!"); //Input bar placeholder tells users they are not connected
        ...
        if (online_status !== undefined && online_status !== "no_connection") {
            $("#send_textarea").attr("placeholder", "输入..."); //on connect, placeholder tells user to type message
