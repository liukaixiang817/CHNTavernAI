...
async function getBookmarkName() {
    ...
    const popupText = `<h3>输入书签名称:<h3>
    <small>留空将会自动生成.</small>`;
...
async function convertSoloToGroupChat() {
    ...
    const name = getUniqueName(`组: ${character.name}`, y => groups.findIndex(x => x.name === y) !== -1);
